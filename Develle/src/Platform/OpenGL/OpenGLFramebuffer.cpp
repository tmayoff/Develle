...
void OpenGLFramebuffer::Invalidate() {
  ...
  if (colorAttachmentSpecifications.size()) {
    ...
    Utils::CreateTextures(multisample, colorAttachments.data(),
    ...
    for (size_t i = 0; i < colorAttachments.size(); i++) {
      ...
      case FramebufferTextureFormat::RGBA8:
        Utils::AttachColorTexture(colorAttachments[i], specification.Samples,
                                  GL_RGBA8, GL_RGBA, specification.Width,
                                  specification.Height, i);
        break;
      case FramebufferTextureFormat::RED_INTEGER:
        Utils::AttachColorTexture(colorAttachments[i], specification.Samples,
                                  GL_R32I, GL_RED_INTEGER, specification.Width,
                                  specification.Height, i);
        break;
      };
    }
  ...
  if (depthAttachmentSpecification.TextureFormat !=
  ...
  if (colorAttachments.size() > 1) {
    ...
    GLenum buffers[4]{GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT2,
  ...
  DV_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) ==
...
int OpenGLFramebuffer::ReadPixel(uint32_t attachmentIndex, int x, int y) {
  DV_CORE_ASSERT(attachmentIndex < colorAttachments.size(),
...
void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value) {
  ...
  glClearTexImage(colorAttachments[attachmentIndex], 0,
                  Utils::DevelleFBTextureFormatToGL(spec.TextureFormat), GL_INT,
                  &value);
}
